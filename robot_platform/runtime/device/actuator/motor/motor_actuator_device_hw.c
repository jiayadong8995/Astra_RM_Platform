...
static platform_device_result_t platform_motor_actuator_init(platform_motor_device_t *device)
{
  ...
  {
    context->joints[i] = get_joint_motor_state(i);
    joint_motor_init(context->joints[i], (uint16_t)(i + 1U), MIT_MODE);
    enable_motor_mode(&hfdcan1, context->joints[i]->para.id, context->joints[i]->mode);
  }

  for (uint8_t i = 0; i < PLATFORM_WHEEL_MOTOR_COUNT; ++i)
  {
    context->wheels[i] = get_chassis_motor_measure_point(i);
  }
